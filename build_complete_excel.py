...
def build_complete_workbook(work_dir: Path | str | None = None, output_path: Path | str | None = None) -> Path:
    target_work_dir = Path(work_dir).resolve() if work_dir else DEFAULT_WORK_DIR
    target_excel_path = Path(output_path).resolve() if output_path else (target_work_dir / "Sampada_September_2026_YouTube_Plan.xlsx")

    # Load metadata, manifest and links
    metadata_rows = []
    meta_csv = target_work_dir / "youtube_metadata.csv"
    if meta_csv.exists():
        with open(meta_csv, "r", encoding="utf-8-sig") as f:
            metadata_rows = list(csv.DictReader(f))

    manifest_rows = []
    manifest_csv = target_work_dir / "manifest.csv"
    if manifest_csv.exists():
        with open(manifest_csv, "r", encoding="utf-8-sig") as f:
            manifest_rows = list(csv.DictReader(f))

    links_rows = {}
    links_csv = target_work_dir / "youtube_links.csv"
    if links_csv.exists():
    ...
    ws_plan["A1"] = "Sampada Magazine – YouTube Publishing Plan"
    ...
    for row_idx, r in enumerate(metadata_rows, start=4):
        ...
        caption_rel = r.get("caption_file", "")
        caption_path = None
        if caption_rel and (target_work_dir / caption_rel).exists():
            caption_path = target_work_dir / caption_rel
        else:
            candidates = [
                target_work_dir / "captions" / f"{Path(r['script_file']).stem}.srt",
                target_work_dir / f"captions/{r['id']}.srt",
            ]
            for cand in candidates:
                if cand.exists():
                    caption_path = cand
                    caption_rel = str(cand.relative_to(target_work_dir))
                    break
        if not caption_rel:
            caption_rel = f"captions/{Path(r.get('script_file', 'script')).stem}.srt"

        caption_text = caption_path.read_text(encoding="utf-8") if caption_path and caption_path.exists() else ""
        preview_srt = "\n".join(caption_text.splitlines()[:12]) if caption_text else ""

        values = [
            r["id"],
            r["title"],
            "Marathi" if r.get("language") == "mr" else "English",
            printed_pages,
            pdf_pages,
            yt_url,
            r.get("keywords", ""),
    ...
    for idx, r in enumerate(metadata_rows, start=1):
        row_num = idx + 3
        script_path = target_work_dir / r["script_file"]
    ...
    ws_qr["A1"] = "QR Code Placement Instructions & Verification"
    ...
    for idx, m in enumerate(manifest_rows, start=4):
        ...
        values = [
            ...
            int(m["printed_start_page"]) if str(m.get("printed_start_page", "")).isdigit() else m.get("printed_start_page", ""),
            int(m["printed_end_page"]) if str(m.get("printed_end_page", "")).isdigit() else m.get("printed_end_page", ""),
            int(m["pdf_start_page"]) if str(m.get("pdf_start_page", "")).isdigit() else m.get("pdf_start_page", ""),
            int(m["pdf_end_page"]) if str(m.get("pdf_end_page", "")).isdigit() else m.get("pdf_end_page", ""),
            "Marathi" if m.get("language") == "mr" else "English",
            m["title"],
        ]
    ...
    try:
        ...
        print(f"Excel workbook updated successfully: {target_excel_path}")
        return target_excel_path
    except PermissionError:
        alt_path = target_work_dir / f"{target_excel_path.stem}_Latest.xlsx"
        wb.save(alt_path)
        print(f"Original file was open. Saved updated copy to: {alt_path}")
        return alt_path
